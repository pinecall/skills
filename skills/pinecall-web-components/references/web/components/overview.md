...
- **Voice messages (WhatsApp/OpenAI-style dictation)** — a **record button** sits next to send in text mode. Tap to record and the server "scribe" **streams the transcription into the input in real time** as you speak; tap again to stop. It's editable text, *not* a call. Language auto-detects (90+ langs + code-switching via ElevenLabs Scribe v2 realtime); pin one with `language="es"` for best accuracy. Authorized with the same `tokenProvider` chat token (no API key in the browser). On by default — hide it with `no-voice-message`. Reusable standalone via `LiveScribe` from `@pinecall/web/scribe`.
- **Conversation history (server-side threads)** — the conversation survives a page refresh. The widget passes a `thread` id over the chat WS; the **server restores the thread on connect**: prior turns seed the agent's real LLM memory and come back as a `chat.history` event, landing in `state.messages` flagged `isHistory`. Only a thread *index* (ids/titles/dates) lives in `localStorage` — message content stays in the platform. Backends can instead seal `threadId` into the token metadata (takes precedence; binds threads to their own users). The header has **new conversation** (+) and **past conversations** (clock menu, last 10 threads). Opt out with `no-history`.

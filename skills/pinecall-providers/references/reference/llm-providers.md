...
```typescript
pc.agent("my-bot", {
  llm: { provider: "openai", model: "gpt-5.4-nano", temperature: 0.3, max_tokens: 256 },
  voice: "elevenlabs/sarah",
  stt: "deepgram/flux",
  prompt: "...",
});
...
// Both expand to:
// { provider: "openai", model: "gpt-5.4-nano", enabled: true }
```

> The legacy `provider:model` format (e.g. `"openai:gpt-5.4-nano"`) still works but is not recommended.
...
| Model | Best for |
|---|---|
| `gpt-5.4-nano` | Most agents — strong reasoning, good cost (recommended default) |
...
llm: { provider: "openai", model: "gpt-5.4-nano", temperature: 0.7, max_tokens: 512 }

// Creative, open-ended
llm: { provider: "openai", model: "gpt-5.4-nano", temperature: 1.0, max_tokens: 1024 }
```
...
agent.update({ llm: "openai/gpt-5.4-nano" });
